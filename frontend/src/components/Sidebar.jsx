    ...
    <div className="w-64 h-screen bg-slate-950 text-white fixed border-r border-slate-800/50 flex flex-col">
      ...
      <div className="px-5 py-6 border-b border-slate-800/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-medium uppercase tracking-wider mt-0.5">{role || "---"}</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        <ul className="space-y-1">
          {menuItem("/dashboard", "Dashboard", icons.dashboard)}
          {menuItem("/appointments", "Appointments", icons.appointments)}

          {/* ADMIN menu */}
          {role === "ADMIN" && (<>
            {sectionLabel("Management")}
            {menuItem("/patients", "Patients", icons.patients)}
            {menuItem("/doctors", "Doctors", icons.doctors)}
            {menuItem("/schedules", "Schedules", icons.schedules)}
            {menuItem("/tests", "Medical Tests", icons.tests)}
            {menuItem("/billing", "Billing", icons.billing)}

            {sectionLabel("Analytics")}
            {menuItem("/reports", "Reports", icons.reports)}
            {menuItem("/activity-log", "Activity Log", icons.activityLog)}

            {sectionLabel("System")}
            {menuItem("/users", "User Management", icons.users)}
