    ...
    <div className="w-64 h-screen bg-slate-950 text-white fixed border-r border-slate-800/50 flex flex-col">
      ...
      <div className="px-5 py-6 border-b border-slate-800/50">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-teal-600 flex items-center justify-center shadow-lg shadow-blue-600/20">
          ...
          <div>
            ...
            <p className="text-[10px] text-blue-400 font-medium uppercase tracking-wider mt-0.5">Patient Portal</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-4 border-b border-slate-800/50">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-500 to-teal-500 flex items-center justify-center text-xs font-bold text-white shadow-sm">
            {initials}
          </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        <ul className="space-y-1">
          {menuItem("/patient/dashboard", "Dashboard", icons.dashboard)}

          {sectionLabel("My Health")}
          {menuItem("/patient/doctors", "Find Doctors", icons.doctors)}
          {menuItem("/patient/appointments", "My Appointments", icons.appointments)}
          {menuItem("/patient/records", "Medical Records", icons.records)}

          {sectionLabel("Billing")}
          {menuItem("/patient/bills", "My Bills", icons.bills)}

          {sectionLabel("Account")}
          {menuItem("/patient/profile", "Profile", icons.profile)}
        </ul>
      </nav>
